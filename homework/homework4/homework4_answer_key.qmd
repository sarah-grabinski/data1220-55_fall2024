...
-   Add packages to the `load_packages` code chunk as you work through the analysis and need their functions. This portion has not been pre-filled for you. If you use the `dplyr` or `tidyverse` packages, make sure that they are the last library you load.
...
**From this summary, you can tell which variables are numeric and which are categorical or text. From the character-type variables, you can see that there are 800 rows or observations in the data. For numerical variables, you can see the minimums and maximums, the values of the quartiles (Q1, Q2/median, Q3), and the mean.** 

**You can also tell that the count of total flights and delayed flights are skewed, as the means are both much larger than the medians. However, the mean and median are approximately equal for `pct_delay` and `avg_delay`. There also appear to be large and small outliers for all numeric variables.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**Like with the `summary()` function from base R,** **you can tell which variables are numeric and which are categorical or text. You can also see the minimums and maximums, the values of the quartiles (Q1, Q2/median, Q3), and the mean.**

**An important feature of this description is that it includes the total number of observations, the number of missing observations, and the number of distinct values in each variable. These metrics give you a clearer idea of the data you have to work with.**

**Again, you can tell from the mean vs median that `flights` and `delayed` are right-skewed, but `pct_delay` and `avg_delay` are not skewed. You also get the values of the 5th, 10th, 90th, and 95th percentiles, which can help give an idea of how close to a normal distribution non-skewed variables are. You also get the lowest and highest 5 values, which highlights any outliers more clearly than the minimum/maximum alone.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**The proper timing of flights is a safety issue, so there is not likely to be much error in the recording of delay times. In addition, there was no missing data in this sample. Finally, the delay time is recorded in seconds, but a flight is considered late if it is delayed over 15 minutes. The time is precisely measured, so there are unlikely to be many flights misclassified as late due to poor measurement. Therefore, it's reasonable to assume that this data is reliable.**

**However, if you inspected your data properly, you might have noticed that you have some extreme values in your data. 2 airports had only 1 flight during a 4-year administration. There were 2 airports with 0% of flights delayed >15 minutes and 2 airports with 100% of flights delayed >15 minutes. There were also 2 airports with an average delay of 0 minutes and 1 airport with an average delay of 222 minutes (3.7 hours!).** 

**Outliers like this are not necessarily indicators that your data is unreliable, but it should give you pause. It could indicate data entry errors or other problems in your sample that warrant further investigation. These could be resolved through some simple data cleaning or wrangling, or you may have to adjust your approach to the statistical analysis if you can't use methods based on the Central Limit Theorem.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**If you calculated your sample statistics correctly, these should be approximately equal to your** $\hat{p}$ **values `phat_trump` (0.168) and `phat_biden`(0.181). When the sample statistic approximately equals the population parameter, your data is valid. Sample statistics calculated from your data are valid estimators of the study population parameters because they are good approximations of the study population.**
...
n_trump * phat_trump >= 10
```

::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**On average, each airport during the Trump administration had 67 delayed flights per year. This is larger than 10.**
:::
...
n_trump * (1 - phat_trump) >= 10
```

::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**On average, each airport during the Trump administration had 330 on time flights per year. This is larger than 10.**
:::
...
n_biden * phat_biden >= 10
```

::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**On average, each airport during the Biden administration had 73 delayed flights per year. This is larger than 10.**
:::
...
n_biden * (1 - phat_biden) >= 10
```

::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**On average, each airport during the Biden administration had 330 on time flights per year. This is larger than 10.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**With 95% confidence, 13.10% to 20.45% of flights were delayed more than 15 minutes at airports during the Trump administration.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**With 95% confidence, 14.38% to 21.90% of flights were delayed more than 15 minutes at airports during the Biden administration.**
:::
...
\
**You could also have said that the null hypothesis is that the two proportions are equal.**
...
\
**You could also have said that the null hypothesis is that the two proportions are not equal.**
...
If your null hypothesis is $H_0\colon \hat{p}_1-\hat{p}_2\ne 0$, you will also need to calculate $\hat{p}_{\text{pool}}$. That's because this hypothesis states that the two samples come from the same population. Therefore, you only need 1 parameter $\hat{p}_{\text{pool}}$ for the population.
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**With 95% confidence, the difference between the proportion of flights delayed more than 15 minutes at airports during the Biden administration and the proportion of flights delayed more than 15 minutes at airports during the Trump administration was -3.9% to 6.6%.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**In your sample, your point estimate (the difference you observed) was that 1.4% more flights were delayed over 15 minutes at airports during the Biden administration than during the Trump administration. This difference is 0.51 standard errors above the null hypothesis that there was no difference in the proportion of flights delayed more than 15 minutes at airports during the two administrations.**

**You could also have stated that the test statistic was less than your critical value `z_diff` (** $\pm$ **1.96) for your confidence interval, using the same** $\alpha=0.05$ **as your significance threshold, so you will fail to reject your null hypothesis.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**The p-value of 0.612 is greater than our significance threshold (alpha) of 0.05, so you should not have rejected the null hypothesis with this data.**

**If the null hypothesis were true, the difference between the proportion of flights delayed at airports during the Trump and Biden administrations was 0. Assuming the null hypothesis is true, the probability of taking two samples of size** $n_{\text{Trump}}=403$ **and** $n_{\text{Biden}}=397$ **from this study population and seeing a probability difference more extreme than the one we got in our sample (greater than 1.4% or less than -1.4%) is 61.2%.**

**In other words, if there were no real difference and you repeated this study over and over, you would get** $\hat{p}_{\text{Biden}}-\hat{p}_{\text{Trump}}>0.014$ **or** $\hat{p}_{\text{Biden}}-\hat{p}_{\text{Trump}}<-0.014$ **about 61.2% of the time. To reject a null hypothesis, we need our observed data to be UNLIKELY under the null hypothesis, or a small p-value. When the observed data is unlikely to occur under the null hypothesis, we can reject that theory of the world and accept the alternate hypothesis. Here, however, our data is consistent with the null hypothesis, so we fail to reject it.**

**The conclusion of this analysis is that this study did not detect a difference between the proportion of flights delayed more than 15 minutes at airports during the Trump and Biden administrations. However, we cannot rule out that there is a difference we could detect if only we had a larger sample and smaller margins of error. For this reason, you should not have said that you accept the null hypothesis.**

**If you made your hypothesis test decision by comparing your test statistic to the critical value under the same alpha, you did not lose points. However, it is better to communicate your results with respect to the the probability of getting a sample like your observed data from the null distribution (the p-value). People with basic statistics knowledge are likely to understand what a p-value is (a probability) than what a critical value is.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**The sample statistic** $\bar{x}_{\text{Trump}}$ **is based on a random sample of 397 airports under the Trump administration, which is more than 30.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**The sample statistic** $\bar{x}_{\text{Biden}}$ **is based on a random sample of 403 airports under the Trump administration, which is more than 30.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**The T distribution is wider than the Z distribution to account for the uncertainty of using a sample statistic (standard deviation** $s$**) versus a population parameter (standard deviation** $\sigma$ **in the standard error calculation. Therefore, you need to go further away from 0 to capture 95% of the data.**

**The values from the Z distribution are smaller because it is a narrower distribution. 95% of the data is closer to the mean, because you don't need to account for the uncertainty of the population standard deviation** $\sigma$ **being unknown. Therefore, critical values will be closer to 0 under the Z distribution than the T distribution for the same confidence level.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**With 95% confidence, flights were delayed 10.82-11.74 minutes on average at airports during the Trump administration.**
:::
...
**You could also have said that the null hypothesis is that the two means are equal.**
...
**You could also have said that the alternate hypothesis is that the two means are not equal.**
...
Use the code chunk below to calculate the lower boundary of your confidence interval for the difference between the average flight delay during the Trump administration and the average flight delay during the Biden administration.

```{r}
mean_diff - t_star_diff * se_mean_diff
```

Use the code chunk below to calculate the upper boundary of your confidence interval for the difference between the average flight delay during the Trump administration and the average flight delay during the Biden administration.
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**With 95% confidence, flights were delayed on average 0.10 to 2.46 minutes longer at airports during the Biden administration than during the Trump administration.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**In your sample, your point estimate (the difference you observed) was that flights were delayed 1.28 minutes longer at airports during the Biden administration than during the Trump administration. This difference is 2.13 standard errors above the null hypothesis that there was no difference in the average flight delay at airports during the 2 administrations.**

**You could also have stated that the test statistic was greater than your critical value `t_star_diff` (**$\pm$ **1.97) for your confidence interval, using the same** $\alpha=0.05$ **as your significance threshold, so you will fail to reject your null hypothesis.**
:::
...
::: {.callout-warning style="color: red" appearance="minimal" icon="false"}
**The p-value of 0.034 is less than your significance threshold of 0.05, so you should have rejected the null hypothesis with the data from your sample.**

**If the null hypothesis were true, the difference between the mean delay time at airports during the Trump and Biden administrations is 0. Assuming the null hypothesis is true, the probability of taking two samples of size** $n_{\text{Trump}}=397$ **and** $n_{\text{Biden}}=403$ **from this study population and seeing a difference in means more extreme than the one we got in our sample (greater than 1.28 minutes or less than -1.28 minutes) is 3.4%.**

**In other words, if there were no real difference and you repeated this study over and over, you would get** $\bar{x}_{\text{Biden}}-\bar{x}_{\text{Trump}}>1.28$ **or** $\bar{x}_{\text{Biden}}-\bar{x}_{\text{Trump}}<-1.28$ **about 3.3% of the time. To reject a null hypothesis, we need our observed data to be UNLIKELY under the null hypothesis, or a small p-value. When the observed data is unlikely under the null hypothesis, we can reject that theory of the world and accept the alternate hypothesis.** 

**Here, we have said that it is evidence against the null hypothesis if the probability of our data under the null hypothesis is less than 5%, therefore, we reject the null hypothesis and accept the alternate hypothesis. The conclusion of this analysis is that flights were delayed longer during the Biden administration than during the Trump administration.**
