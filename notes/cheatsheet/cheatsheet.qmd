...
The null distribution of $\bar{x}$ given the null hypothesis that $\mu=\mu_0$ is $\bar{x} \sim \text{N}\left(\mu_0, SE_{\bar{x}}\right)$. Again, the standard error of $\bar{x}$ is...
...
The $t$-statistic is used to find the probability of your sample having the mean $\bar{x}$ if the null distribution $\text{N}\left(\mu_0, \text{SE}_{\bar{x}}\right)$ were the "true" distribution in your population.
...
A small p-value indicates that the probability of taking a sample of size $n$ with your observed sample mean $\bar{x}$ from the null sampling distribution $\bar{x} \sim \text{N}\left(\mu_0, \text{SE}_{\bar{x}}\right)$ is very low.
