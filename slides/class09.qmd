...
$$
$$
\operatorname{Sample P}(\operatorname{A})=\hat{p}_n=\frac{\operatorname{count}(\operatorname{observation = A})}{\operatorname{count}(\operatorname{observations in sample})}
$$

$$
\operatorname{Population P}(\operatorname{A})=p=\frac{\operatorname{count}(\operatorname{event = A})}{\operatorname{count}(\operatorname{events in sample space})}
$$
...
## Calculating the Population Probability for a Single Event $p$
...
$$
p=\operatorname{P}(\operatorname{Next Song by Chappell Roan})=\frac{\operatorname{count}(\operatorname{Songs By Chappell Roan})}{\operatorname{count}(\operatorname{Total Possible Songs})}=\frac{7}{50}=0.14
$$
...
songs_summary |>
  ...
  labs(x = 'Song by Chappell Roan?', y = 'Proportion of Songs on Playlist', 
       ...
       caption = 'Calculated probability = 0.14')
```

## Calculating the Sample Probability $\hat{p}_n$ for a Single Event

The sample space for the sample probability of the next song being by Chappell Roan when there is "true" shuffle or ***sampling with replacement*** is the number of songs listened to ($n>= 1$).
...
$$
\hat{p}_n=\operatorname{P}(\operatorname{Next Song by Chappell Roan})=\frac{\operatorname{count}(\operatorname{Songs Heard By Chappell Roan})}{\operatorname{count}(\operatorname{Total Songs Heard})}=\frac{x}{n}
$$
